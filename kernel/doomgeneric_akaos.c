...
static unsigned char scancode_to_doom(int scancode) {
    /* This should match DoomGeneric's expectations (see i_input.c in the port):
     * - Some keys map to action codes like KEY_USE/KEY_FIRE instead of ASCII.
     * - Letters are lower-case ASCII. */
    switch (scancode) {
        /* Basics */
        ...
        /* Actions (important for gameplay) */
        case 0x1D: return KEY_FIRE;               /* Ctrl */
        case 0x1D | 0x100: return KEY_FIRE;       /* Right Ctrl */
        case 0x39: return KEY_USE;                /* Space */

        /* Extended arrows (E0-prefixed) */
...
uint32_t DG_GetTicksMs(void) {
    /* PIT is 100 Hz (10ms per tick); Doom expects milliseconds */
    return (uint32_t)(timer_get_ticks() * 10ULL);
}

/* ===== DG_SleepMs — called by i_timer.c ===== */
void DG_SleepMs(uint32_t ms) {
    /* Convert ms to PIT ticks (ceil division, PIT=100Hz) */
